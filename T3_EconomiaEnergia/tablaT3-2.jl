...
factores_ex["Biomasa_Itahue"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_ex["Biomasa_Charrua"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
...
factores_ex["Hidro_Charrua"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_ex["Eolica_LosVilos"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_ex["Solar_Huasco"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_ex["Falla"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
...
factores_nu["N_Hidro_Charrua"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_nu["N_Eolica_Huasco_2"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_nu["N_Eolica_Huasco_3"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_nu["N_Solar_Huasco_2"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_nu["N_Solar_Huasco_3"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_nu["N_Geotermia_DiegodeAlmagro"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
factores_nu["N_Minihidro_Charrua"] =Dict("NOx" => 0.0, "MP" => 0.0, "SOx" => 0.0, "CO2" => 0.0, "Poder Calorifico" => 0, "eficiencia" => 1, "CSMP" =>0,"CSSOx" =>0,"CSNOx" =>0,"CSCO2" =>0)
...
function correr_modelo()
    ...
    + sum(factores_ex[c]["CSMP"]*EmisionesMarginalesExistentes(c,"MP")*contaminante[c]* sum(potencia_central[c,b,f] * DyA(b,f,"MP") for b in bloques, f in keys(abatidor_central))/1000 for c in centrales_existentes)
    + sum(factores_ex[c]["CSNOx"]*EmisionesMarginalesExistentes(c,"NOx")*contaminante[c]* sum(potencia_central[c,b,f] * DyA(b,f,"NOx") for b in bloques, f in keys(abatidor_central))/1000 for c in centrales_existentes)
    + sum(factores_ex[c]["CSSOx"]*EmisionesMarginalesExistentes(c,"SOx")*contaminante[c]* sum(potencia_central[c,b,f] * DyA(b,f,"SOx") for b in bloques, f in keys(abatidor_central))/1000 for c in centrales_existentes)
    + sum(factores_ex[c]["CSCO2"]*EmisionesMarginalesExistentes(c,"CO2")*contaminante[c]* sum(potencia_central[c,b,f] * DyA(b,f,"CO2") for b in bloques, f in keys(abatidor_central))/1000 for c in centrales_existentes)
    + sum(factores_nu[c]["CSMP"]*EmisionesMarginalesNuevas(c,"MP")*contaminante[c]* sum(potencia_central_nueva[c,b,f] * DyA(b,f,"MP")  for b in bloques, f in keys(abatidor_central))/1000 for c in centrales_nuevas)
    + sum(factores_nu[c]["CSNOx"]*EmisionesMarginalesNuevas(c,"NOx")*contaminante[c]* sum(potencia_central_nueva[c,b,f] * DyA(b,f,"NOx") for b in bloques, f in keys(abatidor_central))/1000 for c in centrales_nuevas)
    + sum(factores_nu[c]["CSSOx"]*EmisionesMarginalesNuevas(c,"SOx")*contaminante[c]* sum(potencia_central_nueva[c,b,f] * DyA(b,f,"SOx") for b in bloques, f in keys(abatidor_central))/1000 for c in centrales_nuevas)
    + sum(factores_nu[c]["CSCO2"]*EmisionesMarginalesNuevas(c,"CO2")*contaminante[c]* sum(potencia_central_nueva[c,b,f] * DyA(b,f,"CO2") for b in bloques, f in keys(abatidor_central))/1000 for c in centrales_nuevas)
    )
...
global suma = 0;
suma = sum(factores_ex[c]["Poder Calorifico"] != 0 ?  factores_ex[c]["CSMP"]*EmisionesMarginalesExistentes(c,"MP")*sum(JuMP.value(potencia_central[c,b,f]*DyA(b,f,"MP")) for f in keys(abatidor_central) for b in bloques)/1000 : 0 for c in centrales_existentes) 
       + sum(factores_nu[c]["Poder Calorifico"] != 0 ? factores_nu[c]["CSMP"]*EmisionesMarginalesNuevas(c,"MP")*sum(JuMP.value(potencia_central_nueva[c,b,f]*DyA(b,f,"MP")) for f in keys(abatidor_central) for b in bloques)/1000 : 0 for c in centrales_nuevas)
