...
# Actualizar la capacidad máxima de potencia a poder transmitir por la línea 1-2 a 50[MW].
...
@variable(despacho_economico, dmda_NoSatis[1:num_barras,1:num_periodos] >=0 ) 

# La función objetivo es minimizar los costos de generación
@objective(despacho_economico, Min, sum(generador.GenCost * Q_generacion[generador.IdGen,tiempo] for generador in generadores for tiempo in 1:num_periodos) + 30 * sum(dmda_NoSatis[barra.IdBar,tiempo] for barra in barra_demandas for tiempo in 1:num_periodos))
...
#Balance de potencia para cada barra.
for barra in barra_demandas
    for t in 1:num_periodos
        generacion_barra = isempty([gen for gen in generadores if gen.BarConexion == barra.IdBar]) ? 0 : sum(Q_generacion[gen.IdGen, t] for gen in generadores if gen.BarConexion == barra.IdBar)      
        ...
        if t==1
            @constraint(despacho_economico, dmda_NoSatis[barra.IdBar,t] == barra.Dmd_t1-(generacion_barra - flujo_saliente + flujo_entrante))
        elseif t==2
            @constraint(despacho_economico, dmda_NoSatis[barra.IdBar,t] == barra.Dmd_t2- (generacion_barra - flujo_saliente + flujo_entrante))
        elseif t==3
            @constraint(despacho_economico, dmda_NoSatis[barra.IdBar,t] == barra.Dmd_t3-(generacion_barra - flujo_saliente + flujo_entrante))
        elseif t==4
            @constraint(despacho_economico, dmda_NoSatis[barra.IdBar,t] == barra.Dmd_t4-(generacion_barra - flujo_saliente + flujo_entrante))
        elseif t==5
            @constraint(despacho_economico, dmda_NoSatis[barra.IdBar,t] == barra.Dmd_t5-(generacion_barra - flujo_saliente + flujo_entrante))
        else
            @constraint(despacho_economico, dmda_NoSatis[barra.IdBar,t] == barra.Dmd_t6-(generacion_barra - flujo_saliente + flujo_entrante))
        end               
    end    
end    
...
if termination_status(despacho_economico) == MOI.OPTIMAL
    ...
    for barra in barra_demandas
        for t in 1:6
            if (value.(dmda_NoSatis[barra.IdBar,t]) != 0)
                println("la demanda no satisfecha de la barra ", barra.IdBar, " en el tiempo ", t, " es igual a:", value.(dmda_NoSatis[barra.IdBar,t]))
                for num_linea in lineas
                    println("Flujo de la linea ",num_linea.IdLin, "en el tiempo t", t, "es: ",  value.(flujo[num_linea.IdLin,t]))
                end
            end    
        end
    end    
