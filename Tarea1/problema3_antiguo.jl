...
@variable(despacho_economico, energia_bat[bateria in baterias, t in Time_Aux] >= 0) 
...
@constraint(despacho_economico, constraint_condicion_inicial_bat[bateria in baterias], energia_bat[bateria, 0] == bateria.Cap*bateria.Horas*bateria.E_inicial )
# Restricción Condición final baterias
@constraint(despacho_economico, constraint_condicion_final_bat[bateria in baterias], energia_bat[bateria, length(Time_blocks)] == bateria.Cap*bateria.Horas*bateria.E_final )
...
@constraint(despacho_economico, constraint_cap_potencia_bat[bateria in baterias, tiempo in Time_blocks], -bateria.Cap <= potencia_bat[bateria,tiempo] <= bateria.Cap)
...
if termination_status(despacho_economico) == MOI.OPTIMAL
    ...
    for bateria in baterias
        for tiempo in Time_Aux 
            ...
            elseif (tiempo > 0 )
                println("La bateria " ,bateria.IdBESS, " tiene una energía de: ", value.(energia_bat[bateria,tiempo]), " en el tiempo ", tiempo, "                  y una potencia de ", value.(potencia_bat[bateria, tiempo]))
        end
        end
        println("--------------------------------------------")
    end   
