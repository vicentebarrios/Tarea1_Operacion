...
# Reserva up
@constraint(unit_commitment, Reserva_up[tiempo in Time_blocks], sum(generador.Pmax * estado_gen[generador, tiempo] for generador in generadores) >= sum(barra.Demanda[tiempo] for barra in barras) + Reserva_90[tiempo])
# Reserva down
@constraint(unit_commitment, Reserva_down[tiempo in Time_blocks], sum(generador.Pmin * estado_gen[generador, tiempo] for generador in generadores) <= sum(barra.Demanda[tiempo] for barra in barras) - Reserva_90[tiempo])
